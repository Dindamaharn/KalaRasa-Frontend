...
function UpdateRecipes() {
  ...
  const fetchRecipe = async () => {
    try {
      ...
      /* FIX IMAGE URL */
      if (recipe.gambar) {
        if (recipe.gambar.startsWith("http")) {
          setPreview(recipe.gambar);
        } else {
          setPreview(`${BASE_URL}/storage/${recipe.gambar}`);
        }
      }
      ...
      /* STEPS */
      if (recipe.langkah_langkah) {
        try {
          const parsedSteps = JSON.parse(recipe.langkah_langkah);
          setSteps(parsedSteps);
        } catch (err) {
          console.error("Gagal parse langkah:", err);
          setSteps([]);
        }
      }
    } catch (err) {
